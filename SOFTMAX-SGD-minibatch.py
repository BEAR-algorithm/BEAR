...
def loss_softmax(W, X, y_ind, reg):
    ...
    probability = scores_exp / np.tile(np.sum(scores_exp, axis=1),(np.shape(scores_exp)[1],1) ).T
    ...
    return loss,predicted_class,probability
...
#W = np.zeros((data_dimension,number_of_classes))
W = np.random.random((data_dimension,number_of_classes))
W = W / np.sum(W)
...
while loss > 0.01 and epoch < 2:
    ...
    with open(filepath) as fp:
        while line and cnt< 20242: ##20242
            ...
            if cnt % mini_batch_size == mini_batch_size - 1:
                ...
                WW = W[indicies_in_minibatch_flat,:]
                loss, grad = loss_grad_softmax(WW, X, y_ind, reg=0)
                W[indicies_in_minibatch_flat,:] = W[indicies_in_minibatch_flat,:] - step_size * grad
...
filehandler = open("results/SGD-b5-step01-loss.p", 'wb')
pickle.dump(training_loss_store, filehandler)
filehandler.close()
filehandler = open("results/SGD-b5-step01-cnt.p", 'wb')
pickle.dump(cnt_store, filehandler)
filehandler.close()
filehandler = open("results/SGD-b5-step01-time.p", 'wb')
...
with open(filepath) as fp:
    while line and cnt < 10000 : ##677399
        ...
        if cnt % mini_batch_size == mini_batch_size - 1:
            ...
            WW = W[indicies_in_minibatch_flat,:]
            loss,predicted_class,probability = loss_softmax(WW, X, y_ind, reg=0)
            total_test_loss.append(loss)

            fpr, tpr, thresholds = metrics.roc_curve(np.asarray(data_label)+1, probability[:,1], pos_label=2)
            auc1 = metrics.auc(fpr, tpr)
            auc_TEST = np.append(auc_TEST,auc1)
...
## store
file = open("results/SGD-b5-step01-log.txt","w")
file.write("Average test loss %f \n" %np.mean(np.asarray(total_test_loss)) )
file.write("Average test accuracy %f \n" % acc_val )
file.close()

filehandler = open("results/SGD-b5-step01-test-ground-truth.p", 'wb')
pickle.dump(ground_truth_TEST, filehandler)
filehandler.close()

filehandler = open("results/SGD-b5-step01-test-prediction.p", 'wb')
