...
# Each class has a topk heap. In this function we merge the topk heaps to find
# topk features. We do this by finding the k features that have largest value
# among all classes.
...
step_size = 1 # step size should scale with mini batch size, 100(1), 1(0.01)
reg_param = 0
...
filehandler = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-loss.p", 'wb')
pickle.dump(training_loss_store, filehandler)
filehandler.close()
filehandler = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-cnt.p", 'wb')
pickle.dump(cnt_store, filehandler)
filehandler.close()
filehandler = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-time.p", 'wb')
pickle.dump(time_store, filehandler)
filehandler.close()
filehandler = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-features.p", 'wb')
...
with open(filepath) as fp:
    while line and cnt < 120000 : ## 677399 number of test data points
...
## store
file = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-log.txt","w")
file.write("Average test loss %f \n" %np.mean(np.asarray(total_test_loss)) )
file.write("Average test accuracy %f \n" % acc_val )
file.close()

filehandler = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-test-ground-truth.p", 'wb')
pickle.dump(ground_truth_TEST, filehandler)
filehandler.close()

filehandler = open("results/LBFGS-b2000-step1-m10-w1000-d5-k400-test-prediction.p", 'wb')
