        ...
        while line and cnt< 20242: ## 20242 number of training data points
            ...
            if cnt % mini_batch_size == mini_batch_size - 1:
                ...
                if total_batch_cnt > 0:
                    ...
                    s = sparse.coo_matrix((np.matrix.flatten( - step_size * dd[indicies_in_minibatch_flat,:]).tolist(),(np.matrix.flatten(ind_matrix.astype(int)).tolist(),np.matrix.flatten(col_matrix.astype(int)).tolist())),shape=(data_dimension, 2))
                    ...
                    W[indicies_in_minibatch_flat,:] = W[indicies_in_minibatch_flat,:] - step_size * grad1
                    WW = W[indicies_in_minibatch_flat,:]
                    loss2, grad2 = loss_grad_softmax(WW, X, y_ind, reg=reg_param)
...
filehandler = open("results/LBFGS-b5-step01-m5-loss.p", 'wb')
pickle.dump(training_loss_store, filehandler)
filehandler.close()
filehandler = open("results/LBFGS-b5-step01-m5-cnt.p", 'wb')
pickle.dump(cnt_store, filehandler)
filehandler.close()
filehandler = open("results/LBFGS-b5-step01-m5-time.p", 'wb')
...
## store
file = open("results/LBFGS-b5-step01-m5-log.txt","w")
file.write("Average test loss %f \n" %np.mean(np.asarray(total_test_loss)) )
file.write("Average test accuracy %f \n" % acc_val )
file.close()

filehandler = open("results/LBFGS-b5-step01-m5-test-ground-truth.p", 'wb')
pickle.dump(ground_truth_TEST, filehandler)
filehandler.close()

filehandler = open("results/LBFGS-b5-step01-m5-test-prediction.p", 'wb')
